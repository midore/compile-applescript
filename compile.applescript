...
script mycomplie
	...
	on error_save(new_path)
		set str to new_path & " is exited."
		default_dialog("Not Save", str, {"Done"})
	end error_save
	...
	on get_orgfiles(dir)
		tell application "Finder" to set files_as to every file in the folder dir whose file type is "TEXT" as list
		set ary to {}
...
on run
	tell mycomplie
		try
			...
			# .applescript Files to .scpt File
			compile_files(org_files, new_files)
			giving_dialog("Finish", "See you", {"Bye"})
